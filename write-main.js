...
const content = `// ===== ByAsa - Main Application =====
  ...
  if (currentSearch) {
    ...
    items = items.filter(function(p) {
      return p.title.toLowerCase().indexOf(t) !== -1 ||
        p.description.toLowerCase().indexOf(t) !== -1 ||
        p.category.toLowerCase().indexOf(t) !== -1 ||
        (p.tags || []).some(function(tag) { return tag.toLowerCase().indexOf(t) !== -1; });
    });
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    html += '<div class="product-card animate-fade-in-up" data-id="' + p.id + '">' +
      '<div class="image-wrapper">' +
        ...
        '<button onclick="event.stopPropagation(); var p2=getProductById(\\'' + p.id + '\\'); if(p2&&window.wishlist)window.wishlist.toggle(p2);" title="Wishlist" style="position:absolute;top:12px;right:12px;width:36px;height:36px;border-radius:50%;background:white;box-shadow:0 2px 8px rgba(0,0,0,0.1);display:flex;align-items:center;justify-content:center;border:none;cursor:pointer;z-index:10">' +
